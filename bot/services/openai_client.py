...
class OpenAIClient:
    ...
    async def generate_prompt(
        ...
        user_message = (
            f"Сгенерируй один уникальный промпт для ФОТОРЕАЛИСТИЧНОГО "
            f"профессионального студийного портрета {gender_text} в стиле «{style_text}». "
            f"Результат должен быть НЕОТЛИЧИМ от настоящей фотографии, снятой на профессиональную камеру. "
            ...
            f"Обязательно включи в промпт детали фотореализма: "
            f"текстуру кожи с порами, реалистичные волосы с отдельными прядями, "
            f"натуральные складки и текстуру ткани, настоящее студийное освещение "
            f"с мягкими тенями и бликами на коже. Укажи 'RAW photo, 8K UHD, DSLR' "
            f"в начале промпта. "
